...
# highest occurrence color in the image
highest_occurrence_color = image_to_hashmap(image_path)
print(f"Highest occurrence color: {highest_occurrence_color}")

# Creating a gradient image from the highest occurrence color to black
width, height = 512, 1024  # Increased height to make the image taller
...
# Calculate the position to place the overlay at 35% height of the gradient
overlay_width, overlay_height = overlay_image_obj.size
x_position = (width - overlay_width) // 2  # Center the overlay horizontally
y_position = int(height * 0.35)  # Position at 35% of the gradient height
